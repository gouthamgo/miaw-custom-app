...
export function isChoicesMessage(conversationEntry) {
    if (isConversationEntryChoicesMessage(conversationEntry)) {
        const choicesContent = conversationEntry.content && conversationEntry.content.choices;
        if (!choicesContent) {
            return false;
        }
        return choicesContent.formatType === CONVERSATION_CONSTANTS.FormatTypes.QUICK_REPLIES 
            || choicesContent.formatType === CONVERSATION_CONSTANTS.FormatTypes.BUTTONS;
    }
    return false;
};
...
/**
 * Gets the choices/options from a ChoicesMessage
 * @param {object} conversationEntry
 * @returns {array}
 */
export function getChoicesFromMessage(conversationEntry) {
    if (isChoicesMessage(conversationEntry)) {
        const choicesContent = conversationEntry.content && conversationEntry.content.choices;
        return choicesContent?.optionItems || []; // ← Note: optionItems, not choices
    }
    return [];
}

/**
 * Gets the main text content from a ChoicesMessage
 * @param {object} conversationEntry
 * @returns {string}
 */
export function getChoicesMessageText(conversationEntry) {
    if (isChoicesMessage(conversationEntry)) {
        const choicesContent = conversationEntry.content && conversationEntry.content.choices;
        return choicesContent?.text || "";
    }
    return "";
}
...
/**
 * Validates whether the supplied object is a RoutingResult entry with Transfer type
 * @param {object} conversationEntry
 * @returns {boolean}
 */
export function isTransferEvent(conversationEntry) {
    return conversationEntry.entryType === CONVERSATION_CONSTANTS.EntryTypes.ROUTING_RESULT && 
           conversationEntry.messageType === CONVERSATION_CONSTANTS.RoutingTypes.TRANSFER;
}